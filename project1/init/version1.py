...
class AI(object):
    ...
    def go(self, chessboard):
        ...
        if self.color == COLOR_BLACK:
            _, action = self.max_value(chessboard, 0, self.color)
            if action is not None:
                self.candidate_list.append(action)
        else:
            _, action = self.min_value(chessboard, 0, self.color)
            if action is not None:
                self.candidate_list.append(action)
    ...
    def max_value(self, chessboard, depth, color):
        pos_set = self.valid_pos_set(chessboard, color)
        if self.is_terminate(pos_set) or depth == max_depth:
        ...
        for a in pos_set:
            ...
            v2, _ = self.min_value(nextBoard, depth, -color)
            if v2 > v:
                v = v2
                move = a
        return v, move

    def min_value(self, chessboard, depth, color):
        pos_set = self.valid_pos_set(chessboard, color)
        if self.is_terminate(pos_set) or depth == max_depth:
            return self.curBoardValue(chessboard), None
        v = math.inf
        ...
        for a in pos_set:
            nextBoard = self.nextState(chessboard, a, color)
            v2, _ = self.max_value(nextBoard, depth, -color)
            if v2 < v:
                v = v2
                move = a
        return v, move
